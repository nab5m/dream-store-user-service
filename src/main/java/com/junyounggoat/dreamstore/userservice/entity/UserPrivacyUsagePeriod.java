...
public class UserPrivacyUsagePeriod {
    ...
    private LocalDateTime getUsageEndDateTime(int userPrivacyUsagePeriodCode) {
        if (userPrivacyUsagePeriodCode == UserPrivacyUsagePeriodCode.FOREVER.getCode()) {
            ...
            return this.usageStartDateTime.plusYears(userPrivacyUsagePeriodCode / ONE_YEAR_SECONDS);
        } else if (userPrivacyUsagePeriodCode >= ONE_MONTH_SECONDS) {
            return this.usageStartDateTime.plusMonths(userPrivacyUsagePeriodCode / ONE_MONTH_SECONDS);
        } else {
            return this.usageStartDateTime.plusSeconds(userPrivacyUsagePeriodCode / ONE_MONTH_SECONDS);
        }
    }
}
